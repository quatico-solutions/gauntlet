...
- **`type_and_submit` is how you send a command or message to the program. Use it by default for any line you want the program to act on** — text and the Enter that submits it are delivered as one atomic write, which full-screen TUIs (Ink/React-based ones like Codex, Claude Code) need to avoid dropping the Enter mid-redraw.
