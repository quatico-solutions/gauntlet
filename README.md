...
| `run` | `--silent`, `--format pretty\|jsonl`, `--no-color` | Streaming-transcript controls |
| `batch` | `<story.md> [more.md ...]` | Positional card paths (at least one required) |
| `batch` | `--target <url>` | (required) Application under test |
| `batch` | per-card flags | Same as `run` minus `--out`. Applied uniformly to every card. |
| `batch` | `--silent`, `--format pretty\|jsonl`, `--no-color` | Output mode (see [Batch mode](#batch-mode)) |
